...
<h1>
<?php if ( is_day() ) : ?>
<?php printf( __( 'Daily Archives: %s', 'first-attempt' ), '<span>' . get_the_date() . '</span>' ); ?>
<?php elseif ( is_month() ) : ?>
<?php printf( __( 'Monthly Archives: %s', 'first-attempt' ), '<span>' . get_the_date( 'F Y' ) . '</span>' ); ?>
<?php elseif ( is_year() ) : ?>
<?php printf( __( 'Yearly Archives: %s', 'first-attempt' ), '<span>' . get_the_date( 'Y' ) . '</span>' ); ?>
<?php else : ?>
<?php _e( 'Blog Archives', 'first-attempt' ); ?>
<?php endif; ?>
</h1>
...
<?php _e( 'Nothing Found', 'first-attempt' ); ?></h2>
<div class="postcontent entry-content">
<p><?php _e( 'Apologies, but no results were found for the requested archive. Perhaps searching will help find a related post.', 'first-attempt' ); ?></p>
<?php get_search_form(); ?>
