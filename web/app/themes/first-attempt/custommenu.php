...
class TemplateToaster_custom_Menu extends WP_Widget {
    ...
    // setting the widget
        $widget_ops = array( 'description' => __('Use this widget to add one of your custom menus as a widget.',"first-attempt") );
        parent::__construct( 'nav_menu', __('Custom Menu',"first-attempt"), $widget_ops );
    }
    ...
    function widget($args, $instance){
        ...
        if ( version_compare( $wp_version, '5.8', '>=' ) && strtolower($args['name']) !== 'left sidebar' && strtolower($args['name']) !== 'right sidebar' )
        {
            $style = 'block';
        }
        else
    ...
    function form( $instance) {//form of the widget
        ...
        if ( !$menus ) {
            echo '<p>'. sprintf( __('No menus have been created yet. <a href="%s">Create some</a>.',"first-attempt"), admin_url('nav-menus.php') ) .'</p>';
            return;
        }

        ?>
        <p>
          <label for ="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', "first-attempt") ?></label>
          <input type="text" class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $title; ?>"/>
        </p>

        <p>
          <label for ="<?php echo $this->get_field_id('nav_menu'); ?>"><?php _e('Select Menu:', "first-attempt"); ?></label>
        ...
        <div class="menuoptions">
            ...
            <label for="<?php echo $this->get_field_id('alignment'); ?>"><?php echo(__('Menu Display:',"first-attempt"));?>
            </label>
            <select class ="fullwidth" onchange="color_display(this);" id="<?php echo $this->get_field_id('alignment'); ?>" name="<?php echo $this->get_field_name('alignment'); ?>">
                <option <?php selected($instance['alignment'], 'nostyle');?> value="nostyle"><?php _e('No Style',"first-attempt"); ?>
                </option>
                <option <?php selected($instance['alignment'], 'default');?>value="default"><?php _e('TT Default',"first-attempt"); ?>
                </option>
            </select>
            <br />
            <?php if($instance['alignment']=='nostyle')
            { ?>
             <label class= "cmenu_style" for="<?php echo $this->get_field_id('menustyle'); ?>" style="display:none;"><?php echo(__('Menu Style:',"first-attempt"));?>
            </label>
           ...
           <?php }
            ...
            { ?>
            <label class="cmenu_style" for="<?php echo $this->get_field_id('menustyle'); ?>"><?php echo(__('Menu Style:',"first-attempt"));?>
            </label>
            <select class ="fullwidth cmenu_style" id="<?php echo $this->get_field_id('menustyle'); ?>" name="<?php echo $this->get_field_name('menustyle'); ?>">
          <?php } ?>
                <option <?php selected($instance['menustyle'], 'hmenu');?>value="hmenu"><?php _e('Horizontal Menu',"first-attempt"); ?>
                </option>
                <option <?php selected($instance['menustyle'], 'vmenu');?>value="vmenu"><?php _e('Vertical Menu',"first-attempt"); ?>
                </option>
            </select>
          ...
          <div class="colorpickercontainer">
            <label for="wp-picker-container">
              <?php _e('Active Color', "first-attempt"); ?>
            </label>
          ...
          <div class="colorpickercontainer">
            <label for="wp-picker-container">
              <?php _e('Hover Color', "first-attempt"); ?>
            </label>
            <input class="cw-color-picker" type="text"
          ...
          <div class="colorpickercontainer">
            <label for="wp-picker-container">
              <?php _e('Normal Color', "first-attempt"); ?>
            </label>
            <input class="cw-color-picker" type="text"
...
function TemplateToaster_style() {
    ...
    foreach((array) $wp_registered_sidebars as $index => $value)
    {
        ...
        foreach ( (array) $sidebars_widgets[$index] as $id ) {
            ...
            if($params[0]['widget_name']==__('Custom Menu',"first-attempt")){
